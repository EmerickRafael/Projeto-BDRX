# ============================
# BDRX (BDRs americanas) + ÍNDICE OFICIAL BDRX (B3)
# ============================
...
# --------------------------------
start_date <- as.Date("2022-01-01")  # início do histórico
end_date   <- Sys.Date()             # fim do histórico
Rf_anual   <- 0.0425                 # taxa livre de risco anual (4,25% a.a.)

# --------------------------------
# 2) Carteira teórica atual do BDRX (B3)
#    - Baixa a carteira oficial e normaliza o nome da coluna de data
# --------------------------------
message("Baixando carteira teórica atual do BDRX na B3...")
fetch_marketdata("b3-indexes-current-portfolio", index = "BDRX", throttle = TRUE)

df_bdrx_raw <- indexes_current_portfolio_get() %>%
  filter(index == "BDRX") %>%
  collect()

# Compatibilidade: encontra a coluna de data que a versão do rb3 estiver usando
ref_col <- intersect(c("refdate","reference_date","portfolio_date","date"), names(df_bdrx_raw))
if (length(ref_col) == 0) stop("Não encontrei coluna de data na carteira do BDRX.")
...
# --------------------------------
# 3) Filtrar apenas BDRs de empresas dos EUA
#    - A tabela da B3 não traz país; usamos uma lista de exceções (não-EUA)
# --------------------------------
non_usa_patterns <- c(
  "TSMC","ASML","NVO","SAP","SHOP",
  "BABA|BIDU|PDD|JD|BILI|NTES|IQ",
  "MELI","RIO",
  "AZN|GSK|BTI|HSBC|VOD",
  "NESN|NOVN",
  "ADYEN|HEIA|NN",
  "BASF|BAYN|SIE",
  "LVMH|AIR|OR",
...
df_bdrx <- df_bdrx %>%
  mutate(is_non_usa_guess = str_detect(symbol, paste0("(", paste(non_usa_patterns, collapse="|"), ")")))

tickers_bdrx_usa <- df_bdrx %>%
  filter(!is_non_usa_guess) %>%
  pull(symbol) %>%
  unique()

# --------------------------------
# 4) Históricos de preços das BDRs (Yahoo via yfR)
#    - Baixa apenas tickers com ≥ 75% de dados válidos no período
#    - Organiza em formato wide (datas x tickers)
# --------------------------------
tickers_yahoo <- paste0(tickers_bdrx_usa, ".SA")

message("Baixando históricos das BDRs americanas do BDRX via yfR (Yahoo)...")
yf_prices <- yf_get(
...
# --------------------------------
# 5) Série oficial do índice BDRX (B3)
#    - Usa rb3::index_get() quando disponível; senão, fallback ano a ano
#    - (diferentes versões do rb3 impactam nessa parte)
# --------------------------------
get_bdrx_official <- function(first_date, last_date) {
  if ("index_get" %in% getNamespaceExports("rb3")) {
    return(rb3::index_get("BDRX", first_date = first_date, last_date = last_date) %>% arrange(refdate))
  }
  yrs <- seq(year(first_date), year(last_date))
  for (yy in yrs) {
    rb3::fetch_marketdata("b3-indexes-historical-data", index = "BDRX", year = yy, throttle = TRUE)
  }
  df_raw <- rb3::indexes_historical_data_get() %>% collect()
  if ("index_name" %in% names(df_raw)) df_raw <- df_raw %>% filter(index_name == "BDRX")
  else if ("index" %in% names(df_raw)) df_raw <- df_raw %>% filter(index == "BDRX")
...
bdrx_df  <- get_bdrx_official(start_date, end_date)
bdrx_xts <- xts::xts(bdrx_df$value, order.by = bdrx_df$refdate)
colnames(bdrx_xts) <- "BDRX"
...
# Índice BDRX em base mensal
monthly_bdrx_prices  <- Cl(to.monthly(bdrx_xts, indexAt = "lastof", drop.time = TRUE))

# Retornos mensais LOG por ticker (mantém NAs por coluna)
ret_all <- Return.calculate(monthly_prices_xts, method = "log")

# Retorno mensal LOG do mercado (BDRX) – aqui podemos remover NAs do índice
monthly_market_returns <- na.omit(Return.calculate(monthly_bdrx_prices, method = "log"))
...
# ============================================
# 7) CAPM: Betas por ticker, Rm anual (BDRX), Vol e Sharpe
...
# Beta por ticker: pareia ativo x mercado, remove NAs localmente e exige >= 12 meses
...
# Rm anual (a partir do BDRX no período em LOG → simples)
Rm_log_mensal_med <- mean(as.numeric(coredata(monthly_market_returns)), na.rm = TRUE)
Rm_anual <- exp(12 * Rm_log_mensal_med) - 1
cat(sprintf("Rm anual (BDRX): %.2f%%\n", 100*Rm_anual))

# Retorno esperado anual (CAPM) por ticker — herda NAs de betas com baixa amostra
expected_returns <- Rf_anual + betas * (Rm_anual - Rf_anual)

# Volatilidade anual por ticker: sd de retornos LOG do próprio ticker (>= 12 meses)
...
# Sharpe anual (ex-ante): (E[Ri]-Rf)/σ_i — só válido se beta/ER e vol existirem
...
# ============================================
# 8) ALFA
#    - CAGR anualizado do preço de 2022→hoje por ticker, menos CAPM esperado
...
tab_top20 <- tbl_top20 %>%
  gt() %>%
  tab_header(
    title = md("**Top 20 BDRs — Score Combinado**"),
